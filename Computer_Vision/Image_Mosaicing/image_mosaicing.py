...
def main():
   ...
   for path in dataset_path:
       ...
       # Make sure the images mover left to right
       if inliers[0][0][0] < inliers[0][1][0]:
       ...
       # instad of going the full width w, only got to the point between w and where the warped image begins
           ...
           for j in range(x_offset-1, w):
               if (warped_image[i+y_shift][j+x_shift] == np.array((0, 0, 0))).all() == 0:
                   ...
                   if flag is False:
                       ...
                       temp = (r1 * imgset[0][i][j].astype(np.float32).astype(np.float32) + r2*warped_image[i + y_shift][j + x_shift].astype(np.float32)) / (r1 + r2)
