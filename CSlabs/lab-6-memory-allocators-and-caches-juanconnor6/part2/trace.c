...
void friendlyTrace(int size, int range, int value){
	...
	for(int i=0; i < size; i++){
		char *bin;
		if (i % 10 == 0){
			bin = intToBinaryString((rand()%range),BITWIDTH);
		}else{
			bin = intToBinaryString(value,BITWIDTH);
		}
...
int main(){
	...
	friendlyTrace(ADDRESSES, RANGE, 86);
