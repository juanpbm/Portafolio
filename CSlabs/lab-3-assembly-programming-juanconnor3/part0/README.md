...
1. In the first program we used movl for the values we defined as int. In the second program we used movq for long. 
2. In the first program when rbp is used we use multiples of 4 to allocate memory (int is 4 bytes). In the second one we use multiples of 8 (long is 8 bytes).
...
1. In this program, it uses the edi and esi registers inside the swap function to deliver the parameters.
2. This program primarily uses eax register instead of the ecx register
...
The offset of the address from the first to the last element is 1600. Since an int is 4byts and I have 400 elements, that makes 1600bytes. The first element is at -1600(%rbp) and the last element is at -4(%rbp). Each element is 4bytes away from the next element.
index array = i.
address offset = -(400-i)*4
...
The main difference from the static array is that now the array is not directly in the stack. First, malloc is called with the parameter of the size of the array (1600). Then the address of the first element of the array created by malloc is stored in the stack (%rbp pointer). This time the address of the first element is the smallest address and the last is 1596 more than the first element. Now every time the program wants to access the array it converts the index required in bytes (0 = 0, 1 = 4, 2 = 8...50 = 200...400 = 1596), then it gets the address of the first element from %rbp, adds the value of the required index and modifies its value. 
...
The jump statment used was nop.
...
If the condition is true it doesn't jump it continues executing normally. After all of the code inside the for is processed, it jumps back to the conditional. If the condition is false it jumps to the end of the code and starts doing whatever comes after the for.
...
If the code is short if it has a few cases it's not optimal. It does more cmp than it would do by just comparing one by one. However, if the code is very long, it becomes optimal. It makes a binary search to reduce the amount of cmp the program has to do.  
...
There is an assembly file called [barebones.s](./barebones.s) which contains a minimal assembly program--very similar to the hello world program above. Fill in the questions to make sure you understand all of the parts of an assembly program. Because every instruction in assembly matters, it's important to pay attention to the details of what is going on. I think this is also one of the reasons why assembly programming is fun and very satisfying when things work however!
...
1. Complete the hello world assembly 
2. For compiler generated assembly code analysis, add your .S files that you have generated to this repository.
  - Note this submission will be auto graded for completion (i.e. save the file names as present in the repo).
