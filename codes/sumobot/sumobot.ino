...
const int left#include <Servo.h> // servo library
...
Sensor = 10; // Assign sensor pins for two down sensors
const int rightSensor = 9;
int lsensorState = 0; // Initialize sensor variable
int rsensorState = 0;
const int front_sensor = 0; //declare global variables
...
while (rsensorState == HIGH ||lsensorState == HIGH)
{
  ...
  if(rsensorState == HIGH){//right sensor table edge
    servo_R.write(0);
    servo_L.write(0);
    delay(900);
 }else{
  if(lsensorState == HIGH){ //left sensor table edge 
    servo_R.write(180);
    servo_L.write(180);
    delay(900);
 }
...
//get distance from the front sensor
  volt();
 while (p==0)     //initially keep turning
 {  
  servo_R.write (180);      
  servo_L.write (180);
  stops();
  volt();
    ...
    if (distance <= 6)   //when senses opponent
    {
    p=1;
    }
  }
    while (p==1)        //if opponent sensed, go straight
  {
  servo_R.write (0);
  servo_L.write (180);
  stops();
  volt();
    if (distance >= 6)   //if no longer sensed
    {
    p=0;
    }
  }
